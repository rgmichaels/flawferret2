...
const runGit: GitRunner = async (command, args, options) => {
...
const getExistingRemoteUrl = async (targetRoot: string, runner: GitRunner) => {
  try {
...
export const createGithubRemoteForLocalFramework = async (
  request: CreateFrameworkRequest,
  targetRoot: string,
  { env = process.env, fetcher = fetch, runner = runGit }: GitHubRemoteOptions = {},
): Promise<LocalFrameworkGithubResult> => {
  if (!request.createGithubRepository || request.destinationType !== "local") {
    return {
      message: "GitHub repository creation was not requested.",
      remoteUrl: null,
      repository: null,
      status: "not_requested",
      webUrl: null,
  ...
  if (!token) {
    return {
      message: "GitHub repository creation skipped because GITHUB_TOKEN is not configured.",
      remoteUrl: null,
      repository: null,
      status: "skipped",
      webUrl: null,
  ...
  if (!(await isInsideGitRepository(targetRoot))) {
    return {
      message: "GitHub repository creation skipped because the target directory is not a git repository.",
      remoteUrl: null,
      repository: null,
      status: "skipped",
      webUrl: null,
  ...
  const existingRemoteUrl = await getExistingRemoteUrl(targetRoot, runner);

  if (existingRemoteUrl) {
    return {
      message: "Target repository already has an origin remote.",
  ...
  const authenticatedUser = await githubRequest<GitHubUserResponse>(fetcher, token, "/user", {
    operation: "Unable to read authenticated GitHub user",
  });
  const owner = request.githubOwner.trim() || authenticatedUser.login;
  const repositoryName = request.githubRepository.trim() || repositoryNameFromPackage(request.packageName);
  const isUserRepository = owner.toLowerCase() === authenticatedUser.login.toLowerCase();
  const repository = await githubRequest<GitHubRepositoryResponse>(
    fetcher,
    token,
    isUserRepository ? "/user/repos" : `/orgs/${owner}/repos`,
    {
      body: JSON.stringify({
        auto_init: false,
      ...
      method: "POST",
      operation: `Unable to create GitHub repository ${owner}/${repositoryName}`,
    },
  );
  const branchName = request.githubBranch.trim() || repository.default_branch || "main";

  try {
    await runner("git", ["branch", "-M", branchName], {
      cwd: targetRoot,
    });
    await runner("git", ["remote", "add", "origin", repository.clone_url], {
      cwd: targetRoot,
    });
    await runner("git", ["push", "-u", "origin", branchName], {
      cwd: targetRoot,
    });

    return {
      message: `Created GitHub repository ${repository.full_name} and pushed ${branchName}.`,
    ...
    return {
      message: error instanceof Error ? `GitHub repository was created, but push failed: ${error.message}` : "GitHub repository was created, but push failed.",
      remoteUrl: repository.clone_url,
      repository: repository.full_name,
      status: "failed",
      webUrl: repository.html_url,
...
const coreFiles = (values: FrameworkTemplateRequest): TemplateFileInput[] => [
  ...
  "scripts": {
    ...
    "test:smoke": "NODE_OPTIONS='--import tsx' cucumber-js --tags @smoke",
    "test:headed": "PW_HEADED=true NODE_OPTIONS='--import tsx' cucumber-js",
    "test:api": "NODE_OPTIONS='--import tsx' cucumber-js --tags @api",
    "test:a11y": "NODE_OPTIONS='--import tsx' cucumber-js --tags @a11y",
...
npx pnpm@11.7.0 exec playwright install chromium
cp .env.example .env
npx pnpm@11.7.0 test
\`\`\`

These commands use \`npx pnpm@11.7.0\` so they work even when \`pnpm\` is not installed globally in your shell.
...
- \`npx pnpm@11.7.0 test\` runs all Cucumber scenarios.
- \`npx pnpm@11.7.0 test:smoke\` runs smoke coverage.
- \`npx pnpm@11.7.0 test:api\` runs API-tagged scenarios.
- \`npx pnpm@11.7.0 test:a11y\` runs accessibility-tagged scenarios.
- \`npx pnpm@11.7.0 typecheck\` validates TypeScript.
...
Then("the page should be served from the configured base URL", async function (this: TestWorld) {
  const configuredUrl = new URL(env.BASE_URL);
  const currentUrl = new URL(getPage(this).url());
  const configuredHost = configuredUrl.hostname.replace(/^www\\./, "");
  const currentHost = currentUrl.hostname.replace(/^www\\./, "");

  expect(currentUrl.protocol).toBe(configuredUrl.protocol);
  expect(currentHost).toBe(configuredHost);
});
`,
  },
];
...
const getExistingGitHubPaths = async (
  ...
  const tree = await githubRequest<GitHubTreeResponse>(
    fetcher,
    token,
    `/repos/${repositoryPath}/git/trees/${baseTreeSha}?recursive=1`,
    {
      operation: `Unable to inspect GitHub repository tree for ${repositoryPath}`,
    },
  );
...
// GitHub reports a taken repository name as a 422 with "name already exists on this account" in the
// validation errors array. githubRequest surfaces the raw response body, so match on that text and
// re-throw something the wizard's createError banner can show as-is.
const repositoryNameTakenPattern = /already exists on this account|name already exists/i;

// Creates the destination repository up-front for the "Create a new GitHub repository" option, so
// the branch -> tree -> commit -> pull request flow below runs unchanged against a repo that now
// exists. Returns null for every other build, leaving the existing-repo flow untouched.
const createGithubRepositoryForFramework = async (
  fetcher: GitHubFetch,
  token: string,
  request: CreateFrameworkRequest,
): Promise<GitHubRepositoryResponse | null> => {
  if (!request.createGithubRepository || request.destinationType !== "github") {
    return null;
  }
  ...
  const owner = request.githubOwner.trim();
  const repositoryName = request.githubRepository.trim() || repositoryNameFromPackage(request.packageName);
  const isUserRepository = !owner || owner.toLowerCase() === authenticatedUser.login.toLowerCase();

  try {
    return await githubRequest<GitHubRepositoryResponse>(
      fetcher,
      token,
      isUserRepository ? "/user/repos" : `/orgs/${owner}/repos`,
      {
        body: JSON.stringify({
          // auto_init gives the new repo an initial commit and a default branch; without one there
          // is no base branch to read, no base tree to build on, and no branch to open a PR against.
          auto_init: true,
          description: `Playwright, TypeScript, and Cucumber test framework generated by FlawFerret 2.`,
          name: repositoryName,
          private: request.githubRepositoryVisibility !== "public",
        }),
        method: "POST",
        operation: `Unable to create GitHub repository ${owner || authenticatedUser.login}/${repositoryName}`,
      },
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);

    if (repositoryNameTakenPattern.test(message)) {
      throw new Error(
        `GitHub repository ${owner || authenticatedUser.login}/${repositoryName} already exists. Pick a different repository name, or clear "Create a new GitHub repository" to commit into the existing repository.`,
      );
    }
...
export const createFrameworkPullRequest = async (
  ...
  // Repo creation happens before any file work, so a rejected name (already taken, no org access)
  // fails the build without leaving a half-populated repository behind.
  const createdRepository = await createGithubRepositoryForFramework(fetcher, token, request);
  // A brand-new repo's only content is the auto_init README stub, so overwrite it rather than
  // reporting the generated README as "skipped" on a repository the user just created.
  const overwriteExisting = request.overwriteExisting || createdRepository !== null;
  // GitHub picks the default branch name on auto_init (the account/org setting, not the wizard's
  // Branch field), so target whatever it actually created.
  const baseBranch = createdRepository?.default_branch?.trim() || request.githubBranch;
  // A freshly created repo is authoritative about its own owner/name (the owner can resolve to the
  // authenticated user, and the name can fall back to the package name), so prefer its full_name.
  const repositoryPath = createdRepository?.full_name || `${request.githubOwner}/${request.githubRepository}`;
  ...
  const branch = await githubRequest<GitHubCommitResponse>(
    fetcher,
    token,
    `/repos/${repositoryPath}/branches/${encodeURIComponent(baseBranch)}`,
    {
      operation: `Unable to read GitHub branch ${baseBranch} in ${repositoryPath}`,
    },
  );
  const existingPaths = await getExistingGitHubPaths(fetcher, token, repositoryPath, branch.commit.tree.sha);
  ...
  await githubRequest<GitHubRefResponse>(fetcher, token, `/repos/${repositoryPath}/git/refs`, {
    body: JSON.stringify({
  ...
  await githubRequest<GitHubRefResponse>(
    fetcher,
    token,
    `/repos/${repositoryPath}/git/refs/heads/${encodeGitHubPathPart(branchName)}`,
    {
      body: JSON.stringify({
        sha: commit.sha,
      }),
...
export const createFrameworkFiles = async (request: CreateFrameworkRequest): Promise<CreateFrameworkResponse> => {
  ...
  const localGit = await initializeLocalGitRepository(targetRoot, request.initializeGitRepository);
  const githubRemote = await createGithubRemoteForLocalFramework(request, targetRoot);
